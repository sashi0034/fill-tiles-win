...
namespace inGame::effect
{
    ...
    SakuraFormation::SakuraFormation(EffectManager *effectManager, const Vec2<double> &pos)
    ...
    CoroTask SakuraFormation::produceEffectAsync(CoroTaskYield &yield, EffectManager *manager, const Vec2<double> pos)
    {
        ...
        lua->RunCoroutine(
                ...
                [&](double x, double y)->void {manager->GetChildren()->Birth(new SakuraFormation(manager, pos + Vec2{x, y}));}
        );
